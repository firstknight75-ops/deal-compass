 ...
 * + Caching on crawl results + sources
 */
...
export class TradeRadarService extends BaseService {
  ...
  async getActiveSources(): Promise<CrawlSource[]> {
    return getOrSet('radar:active_sources', async () => {
  ...
  async runCrawlForSource(sourceId: string): Promise<{ recordsProcessed: number; newRecords: number }> {
    const cacheKey = `radar:crawl:${sourceId}`;
    ...
    }, 600); // 10 min cache for crawl results
  }

  // Invalidation helper (call after manual refresh)
  async invalidateCrawlCache(sourceId?: string) {
    if (sourceId) {
      cache.delete(`radar:crawl:${sourceId}`);
    } else {
      cache.delete('radar:active_sources');
    }
  }
